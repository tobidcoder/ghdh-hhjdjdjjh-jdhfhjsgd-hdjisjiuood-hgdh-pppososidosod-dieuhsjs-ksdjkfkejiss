  ...
  try {
    ...
    try {
      database.transaction(() => {
        ...
        `
          ...
          .run('product_sync', 1, 1, 1, new Date().toISOString(), products.length)
      })()
    ...
    try {
      ...
      `
        ...
        .run('product_sync', 1, 1, 0, new Date().toISOString(), 0)
    } catch (progressError) {
...
export function getProductSyncProgress(): ProductSyncProgress | null {
  const database = getDatabase()
  return (
    (database.prepare('SELECT * FROM product_sync_progress WHERE id = ?').get('product_sync') as
      | ProductSyncProgress
      | undefined) || null
  )
}
